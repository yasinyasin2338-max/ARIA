...
app.get('/api/health',(_q,res)=>res.json({ok:true,service:'ARIA ULTIMATE',version:'7.1-neural-voice',provider:openAI?'OpenAI':openRouter?'OpenRouter+AI Horde':'AI Horde',freeMode:!openAI,capabilities:{chat:true,webSearch:true,imageGeneration:true,imageEditing:true,voiceInput:true,voiceOutput:true,neuralPersianTts:true,video:Boolean(process.env.ARIA_VIDEO_ENDPOINT),memory:true,android:true}}));
...
app.get('/api/capabilities',(_q,res)=>res.json({chat:true,webSearch:true,imageGeneration:true,imageEditing:true,voiceInput:true,voiceOutput:true,neuralPersianTts:true,video:Boolean(process.env.ARIA_VIDEO_ENDPOINT),memory:true,android:true,backgroundVoice:true,freeMode:true}));
