...
public class MainActivity extends Activity {
    ...
    private void buildWeb(){web=new WebView(this);web.setLayoutParams(new ViewGroup.LayoutParams(-1,-1));web.setBackgroundColor(0xff212121);WebSettings s=web.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setMediaPlaybackRequiresUserGesture(false);s.setAllowFileAccess(true);s.setAllowContentAccess(true);s.setSupportZoom(false);
        webTts=new TextToSpeech(this,status->{if(status==TextToSpeech.SUCCESS){try{webTts.setLanguage(new Locale("fa","IR"));webTts.setSpeechRate(1.03f);webTts.setPitch(1.08f);}catch(Exception ignored){}}});
        web.addJavascriptInterface(new Object(){@android.webkit.JavascriptInterface public void speak(String text){runOnUiThread(()->{if(webTts!=null){try{webTts.speak(text,TextToSpeech.QUEUE_FLUSH,null,"aria-web");}catch(Exception ignored){}}});}},"ARIA");
    ...
    @Override protected void onDestroy(){try{unregisterReceiver(receiver);}catch(Exception ignored){}if(webTts!=null){try{webTts.stop();webTts.shutdown();}catch(Exception ignored){}}if(web!=null)web.destroy();super.onDestroy();}
}
